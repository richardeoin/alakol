...
class AlakolCommand:
    ...
    def command(self, motor_speed_percent = 0, close_valve = False):
        if type(motor_speed_percent) is not int or \
           motor_speed_percent < 0 or motor_speed_percent > 100:
            raise(Exception("Invalid motor speed percentage"))

        requests.post(f"{self.url}command", data = {
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--motor',
                        dest='motor', default=0,
                        help='Motor speed percentage (default = 0%)')
    parser.add_argument('-c', '--close-valve',
                        dest='valve', action='store_true',
